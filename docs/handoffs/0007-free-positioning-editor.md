...
## 実装結果
- 内容要素をクリック選択し、Konva Transformerでドラッグ／リサイズ可能にした。空白クリックとEscで選択解除する。
- textの変形は `width/height/fontSize` へ反映してscaleを1へ戻す。写真・校章・印は `width/height` へ反映する。
- カード端／中央と他の内容要素の端／中央へ5px以内でスナップし、操作中だけガイドラインを表示する。
- 背景／帯／透かしは固定のまま。選択枠とガイドはPNG書き出し時に非表示にする。
- Vitestでスナップ／変形計算を、Playwrightで選択／ドラッグ／ガイド／リサイズ／解除／リセットを検証する。
- スコープ外のルーラー、写真トリミング、生成機能、複数テンプレは未実装。
