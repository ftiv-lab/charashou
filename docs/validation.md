...
- `npm run e2e`：Playwright最小スモーク。Canvas画像差分、装飾プリセットの保存復元とPNG反映、写真調整、内容要素の選択／ドラッグ／リサイズ／ホバー、text/写真/校章の右インスペクタ、Undo/Redo、IndexedDB保存／自動復元／保存カードCRUD、JSON往復・不正JSON、カード寸法、Blob経由の3倍PNG download。
