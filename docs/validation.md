...
- `npm run e2e`：Playwright最小スモーク。Canvas画像差分、写真調整、内容要素の選択／ドラッグ／リサイズ、テキスト右インスペクタ、Undo/Redo、IndexedDB保存／自動復元／保存カードCRUD、JSON往復・不正JSON、カード寸法、Blob経由の3倍PNG download。
