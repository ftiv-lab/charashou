...
- `npm run e2e`：Playwright最小スモーク。Canvas画像差分、背景地紋の種類/パラメータ編集・保存復元・PNG反映、装飾プリセット、写真調整、内容要素の選択／ドラッグ／リサイズ／ホバー、text/写真/校章の右インスペクタ、Undo/Redo、IndexedDB保存／自動復元／保存カードCRUD、JSON往復・不正JSON、カード寸法、Blob経由の3倍PNG download。
...
- ブラウザ検証はPlaywrightを既定経路とし、挙動は `npm run e2e`、レビュー画像は `npm run shots` で確認する。
- in-app Browserは対話的なDOM調査や共有画面での目視確認が必要な場合の補助。利用不能／接続不調でもブロッカー扱いせずPlaywrightへ戻る。
- Playwrightが受け入れ基準を覆っていれば、in-app Browserの実行はDoneの必須条件ではない。
