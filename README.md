...
### カスタマイズ
「デザイン」タブでテーマ色を変更し、氏名のフォントと文字サイズも変更した状態。

![テーマと文字スタイルを変更したカード](docs/screenshots/02-customize.png)

### 写真調整
「写真」タブで自前生成の架空キャラクター画像を入れ、ズームと位置を調整した状態。
...
2. 左パネルの「内容／デザイン／写真」を切り替えて編集するか、カード上のテキストを選択して右インスペクタから内容・位置・文字サイズ・色を編集する。選択要素はドラッグ／リサイズも可能。
...
- 仕組み：テンプレJSONの座標要素をKonva Stageへ描画。内容要素は選択・移動・リサイズでき、テキストは選択追従の右インスペクタからも同じtemplate stateを編集する。
