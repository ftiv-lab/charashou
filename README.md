...
オリジナルキャラの**身分証（学生証）風カード**を作るブラウザツール。
顔写真・名前・学校名・学年などを差し替えて、PNGで書き出せる。

> **コンセプト（2026-06-13 確定）**：創作キャラの**身分証メーカー**。同人/オリキャラの**設定資料集・プロフィールに貼るIDカード**を作る。まず学生証を深掘りし、社員証→ギルド/魔法学校へ拡張予定。
> 想定ユーザー＝**AI画像生成は使えるが、自分でデザインを作るのは苦手**な層。装飾は「背景地紋/透かし/印＝アプリ生成、校章/紋章＝アップロード＋AIプロンプト補助」で役割分担。
...
- [ ] 丸印（ハンコ）ジェネレーターの編集UI
- [ ] 校章/印/背景のアップロード＋AI生成プロンプト補助
- [ ] 複数テンプレ（学生証深掘り→社員証→ギルド/魔法学校）＋テンプレ毎の項目＋切替
