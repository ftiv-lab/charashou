...
2. 左パネルの「内容／デザイン／写真」を切り替えて編集するか、カード上の文字・写真・校章・印を選択して右インスペクタから詳細を編集する。選択要素はドラッグ／リサイズも可能。
...
- 仕組み：テンプレJSONの座標要素をKonva Stageへ描画。内容要素は選択・移動・リサイズでき、右インスペクタも同じtemplate/photo stateを直接編集する。
